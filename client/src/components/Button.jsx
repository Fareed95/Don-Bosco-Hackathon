...
export function TailwindcssButtons({ onCopy, buttons }) {
  return (
    <div className="pb-40 px-4 w-full">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full max-w-7xl mx-auto gap-10">
        {buttons.map((button, idx) => (
          <ButtonsCard key={idx} onClick={() => onCopy(button)}>
            {button.component}
          </ButtonsCard>
        ))}
      </div>
...
// Export button definitions separately so they can be managed by parent components
